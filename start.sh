...
export WINE_PYTHON="${WINE_PYTHON:-C:\Program Files\Python39\python.exe}"
...
# Ensure MT5 portable mode and sync configs
mt5_dir="/config/.wine/drive_c/Program Files/MetaTrader 5"
touch "$mt5_dir/portable.txt" 2>/dev/null || true
if [ -d "$mt5_dir/Config" ]; then
    mkdir -p "$mt5_dir/config" 2>/dev/null || true
    for f in servers.dat accounts.dat terminal.lic common.ini; do
        if [ -f "$mt5_dir/Config/$f" ] && [ ! -f "$mt5_dir/config/$f" -o "$mt5_dir/Config/$f" -nt "$mt5_dir/config/$f" ]; then
            cp -f "$mt5_dir/Config/$f" "$mt5_dir/config/$f" 2>/dev/null || true
        fi
    done
fi
chown -R abc:abc "$mt5_dir" 2>/dev/null || true
...
if ! $wine_executable "$WINE_PYTHON" -c "import MetaTrader5, rpyc" 2>/dev/null; then
    show_message "[6/7] Installing Wine Python dependencies (MetaTrader5, rpyc, numpy)..."
    $wine_executable "$WINE_PYTHON" -m pip install --no-cache-dir "numpy<2" "rpyc==5.3.1" "MetaTrader5"
else
...
while true; do
    ...
    wine "${WINE_PYTHON:-C:\Program Files\Python39\python.exe}" /config/server.py >> /config/server.log 2>&1
