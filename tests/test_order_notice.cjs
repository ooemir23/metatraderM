...
(async()=>{
 ...
 let release,calls=0;
 const ctx=vm.createContext({console,Date,AbortController,document:{addEventListener(){},getElementById:node,querySelectorAll:()=>[]},window:{addEventListener(){}},setInterval(){},setTimeout(){},clearTimeout(){},
 ...
 release({ok:false,json:async()=>({detail:'Market closed',retcode:10018})});await request;
 ...
 release({ok:false,json:async()=>({detail:'Not enough money',retcode:10019})});await pending;
