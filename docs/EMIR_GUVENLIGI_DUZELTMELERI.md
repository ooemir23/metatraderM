...
- Günlük limit mevcut panel ayarından alınır; başlangıç varsayılanı `DAILY_LOSS_LIMIT=500`. Güncel kuralda AI emri için hesap limitiyle AI ayarındaki limitin düşüğü kullanılır; AI ayarı manuel ve Python HMA emirlerinin limitini değiştirmez. Sınıra ulaşmak mevcut pozisyonları otomatik kapatmaz.
...
- Ayrı MQL5 EA'da Telegram kaldırıldı, ters kapatma sonucu doğrulanmadan yeni emir açılması engellendi; Buy/Sell dönüş kodu kontrolü eklendi. **Bağımsız EA, web uygulamasının SQLite günlüğünü ve günlük zarar limitinin kalıcı ayarını paylaşmaz.** Güncel EA kendi broker risk kontrolünü uygular; ayarlar iki uygulamada ayrıca eşitlenmelidir.
...
## Güncel risk kuralı — 25 Eylül 2026

- Panelde kaydedilen günlük zarar limiti hesap genelindeki manuel ve HMA emirlerini sınırlar. AI emrinde bu hesap limitiyle AI ayarındaki limitin **daha düşüğü** uygulanır. AI ayarı artık diğer emirlerin limitini değiştirmez. Bu bölüm, yukarıdaki ilk aşama açıklamasındaki eski AI varsayılanı davranışının yerini alır.
- Kârdaki/zarardaki toplu kapatma, pozisyonun güncel açık kârı ve swapı ile aynı pozisyon kimliğine bağlı geçmiş deal'lerin komisyon ve fee toplamını kullanır. Önceki kısmi kapanışların gerçekleşmiş kârı, kalan açık hacmi kârda göstermez. Geçmiş okunamazsa güvenli sınıflama yapılamadığı için filtreli kapatma başlamaz. Brokerın pozisyon kimliğine bağlamadığı ayrı maliyet hareketleri bu hesapta bulunmayabilir.
- Risk önizlemesi günlük zarar, uygulama lot/işlem sınırları ve SL mesafesi engellerini gösterir. Fiyat ve broker durumu emir anında yeniden kontrol edilir.
- Bağımsız MQL5 EA artık varsayılan olarak yalnız sinyal üretir. Otomatik işlem açıkça etkinleştirilirse kendi içinde hesap genelindeki günlük zarar, lot ve işlem sayısı sınırlarını broker verisinden denetler. EA web panelinin SQLite durdurma kilidine ve emir kimliği günlüğüne bağlı değildir. Aynı hesapta web otomasyonuyla eşzamanlı çalıştırılmamalıdır; ortak atomik koordinasyon sağlanmış sayılmaz.
